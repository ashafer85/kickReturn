...
class KRGame {
  ...
  handleEnd() {
    ...
    if (this.winner === true) {
      ...
      this.ctx.fillStyle = 'black';
      this.ctx.font = '40px Arial';
      this.ctx.fillText('Your Return: ' + (100 - this.y2S) + ' yards', (60), 80);
    }
  }
}
...
class KickReturner {
  ...
  render() {
    ...
    if (this.styleHelmet === true) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(this.x, this.y, this.radius, (this.dirVector - (Math.PI/4)), (this.dirVector + (Math.PI/4)), false);
      this.ctx.fillStyle = this.colorLight;
      this.ctx.fill();
      this.ctx.closePath();
    }
