...
document.addEventListener('DOMContentLoaded', () => {
  ...
  game.play();
  setInterval(game.moveDefense, 50);
});
...
class KRGame {
  constructor(krCanvas, ctx) {
    ...
    this.kR = new __WEBPACK_IMPORTED_MODULE_0__kick_returner_js__["a" /* default */](this.krCanvas, this.ctx);
  ...
  checkPursue(def) {
    if ( this.spreadPyth(def.returner.x, def.returner.y, def.x, def.y) <= (300) || ((def.returner.y - 150) < def.y)) {
      def.pursue = true;
    } else {
      def.pursue = false;
    }
...
class Defender {
  ...
  constructor(krCanvas, ctx, returner){
    ...
    this.y = krCanvas.height - (krCanvas.height - (30 + 100*Math.random()));
    this.radius = 16;
    this.dirStart = 0;
    this.dirEnd = this.dirStart + Math.PI;
    this.dirVector = this.dirStart - Math.PI/2;
    this.delta = 1 + Math.ceil(3*Math.random());
