...
class KRGame {
  ...
  handleEnd() {
    ...
    if (this.winner === true) {
      this.ctx.drawImage(this.refTD, (60 + this.kR.onLeft), 100, 200, 200);
      ...
      this.ctx.font = '20px Arial';
      this.ctx.fillText('Your Return: ' + (100 - this.y2S) + ' yards', (60 + this.kR.onLeft), 380);
    } else {
      this.ctx.drawImage(this.refDB, (60 + this.kR.onLeft), 100, 200, 250);
      this.ctx.fillStyle = 'red';
      this.ctx.font = '40px Arial';
...
class KickReturner {
  ...
  render() {
    ...
    //right side
    this.ctx.beginPath();
    this.ctx.arc(this.x, this.y, this.radius, (this.dirVector + (Math.PI/16)), (this.dirStart + (Math.PI/4)), this.pastDef);
    this.ctx.fillStyle = this.colorLight;
    this.ctx.fill();
    this.ctx.closePath();
    this.ctx.beginPath();
    this.ctx.arc(this.x, this.y, this.radius, (this.dirVector - (Math.PI/16)), (this.dirEnd - (Math.PI/4)), !this.pastDef);
    this.ctx.fillStyle = this.colorLight;
    this.ctx.fill();
    this.ctx.closePath();
    // Blue super Sides to create yellow side stripes
    this.ctx.beginPath();
    this.ctx.arc(this.x, this.y, this.radius, (this.dirVector + (7*Math.PI/64)), (this.dirStart + 3*Math.PI/16), this.pastDef);
    this.ctx.fillStyle = this.colorDark;
    this.ctx.fill();
    this.ctx.closePath();
    this.ctx.beginPath();
    this.ctx.arc(this.x, this.y, this.radius, (this.dirVector - (7*Math.PI/64)), (this.dirEnd - 3*Math.PI/16), !this.pastDef);
    this.ctx.fillStyle = this.colorDark;
    this.ctx.fill();
    this.ctx.closePath();
    //Yellow front
    this.ctx.beginPath();
    this.ctx.arc(this.x, this.y, this.radius, (this.dirVector - (3*Math.PI/8)), (this.dirVector + (3*Math.PI/8)), this.pastDef);
    this.ctx.fillStyle = this.colorLight;
    this.ctx.fill();
    this.ctx.closePath();
